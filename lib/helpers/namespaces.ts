import namespace from '@rdfjs/namespace'

export const schema = namespace('https://schema.org/')
export const rdf = namespace('http://www.w3.org/1999/02/22-rdf-syntax-ns#')
export const sh = namespace('http://www.w3.org/ns/shacl#')
export const xsd = namespace('http://www.w3.org/2001/XMLSchema#')
export const rdfs = namespace('http://www.w3.org/2000/01/rdf-schema#')
export const shFrm = namespace('https://github.com/danielbeeke/shacl-form/ontology#')
export const dash = namespace('http://datashapes.org/dash#')
export const shn = namespace('https://schemas.link/shacl-next#')
export const editorjs = namespace('https://editorjs.io/')